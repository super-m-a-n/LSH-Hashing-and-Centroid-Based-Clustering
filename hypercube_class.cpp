...
hypercube::hypercube(){
    this->cube_array = new std::list<const Object*>[1 << d1]; //size 2^d1 (di is the global variable which stores the dimension of the hypercube)
...
bool hypercube::execute(const Dataset & dataset, Dataset & query_dataset, const std::string & output_file, const int & N, const int & R, double (*metric)(const Object &, const Object &)){
	...
	for (int i = 0; i < num_of_Objects; i++)		// run for each of the query Objects
	{
		...
		std::vector <std::pair <double, const Object*> > appr_nearest = this->appr_nearest_neighbors(dataset, query_dataset.get_ith_object(i), file, N, metric);
		// end timer for lsh
	    ...
	    file << "tCube : " << tCube.count() << "ms\n";
	    file << "tTrue : " << tTrue.count() << "ms\n\n";
		
		file << "tCube / tTrue: " << (double) tCube.count() / (double) tTrue.count() << std::endl;
...
void hypercube::vertex_visiting_first_stage(search_type Type, const int R, int curr_vertex, int ham_dist, int M_rem, int probes_rem, uint curr_bit,
	std::priority_queue <std::pair <double, const Object*> >& max_heap,
								const Object & query_object, std::ofstream & file, double (*metric)(const Object &, const Object &), int N){

	this->vertex_visiting_second_stage(Type, R, curr_vertex, M_rem, probes_rem, curr_bit, ham_dist, max_heap, query_object, file, metric, N);
	...
	this->vertex_visiting_first_stage(Type, R, curr_vertex, ham_dist +1, M_rem, probes_rem, curr_bit, max_heap, query_object, file, metric, N);
...
void hypercube::vertex_visiting_second_stage(search_type Type, const int R, int curr_vertex, int& M_rem, int& probes_rem, uint curr_bit, int ham_rem,
	...
	if (ham_rem == 0){
		this->vertex_visiting_third_stage(Type, R, curr_vertex, M_rem, max_heap, query_object, file, metric, N);
		probes_rem -= 1;
		return;
	}
	...
	if ((curr_bit >> ham_rem) != 0){
		this->vertex_visiting_second_stage(Type, R, curr_vertex, M_rem, probes_rem, curr_bit >> 1, ham_rem, max_heap, query_object, file, metric, N);
		if (M_rem == 0 || probes_rem == 0) return;
	}

	this->vertex_visiting_second_stage(Type, R, curr_vertex xor curr_bit, M_rem, probes_rem, curr_bit >> 1, ham_rem -1, max_heap, query_object, file, metric, N);
}
...
void hypercube::vertex_visiting_third_stage(search_type Type, const int R, int curr_vertex, int& M_rem,
	...
	for(auto const& obj_p : vertex_list){
		...
		if (Type == kNN){
			...
			push_at_most_N(obj_p, N, dist, max_heap);
		}
		else if (dist < R){
			file << "Range Search: Point-Object " << obj_p->get_name() << '\n';
		}
		M_rem -= 1;
		if (M_rem == 0) return;
	}
...
std::vector <std::pair <double, const Object*> > hypercube::appr_nearest_neighbors(const Dataset & dataset, const Object & query_object, std::ofstream & file, const int & N, double (*metric)(const Object &, const Object &))
{
	...
	this->vertex_visiting_first_stage(kNN, 0, query_vertex, 0, M, probes, 1 << (d1-1), max_heap, query_object, file, metric, N);
...
void hypercube::range_search(const Object & query_object, std::ofstream & file, const int & R, double (*metric)(const Object &, const Object &))
{
	...
	this->vertex_visiting_first_stage(RANGE_SEARCH, R, query_index, 0, M, probes, 1 << (d1-1), max_heap, query_object, file, metric,0);
